        ...
        {navLinks.map((item, key) => {
          ...
          return (
            <Link
              key={key}
              to={item.path}
              className={`flex items-center space-x-3 rounded-md px-3 py-2 text-sm font-medium text-[#F8FAFC] ${pathname === item.path ? "bg-accent" : "transition-colors hover:bg-accent hover:text-primary"} `}
            >
